...
Widget build(BuildContext context) {
  ...
  Widget _buildDurationSettings(ScheduleProvider provider) {
    return Column(
      children: [
        ListTile(
          title: const Text('Short Bell Duration (sec)'),
          trailing: SizedBox(
            width: 60,
            child: TextField(
              controller: TextEditingController(
                text: provider.shortBellDuration.toString()),
        ...
        ListTile(
          title: const Text('Long Bell Duration (sec)'),
          trailing: SizedBox(
            width: 60,
            child: TextField(
              controller: TextEditingController(
                text: provider.longBellDuration.toString()),
              keyboardType: TextInputType.number,
  ...
  Widget _buildBellModeSettings(ScheduleProvider provider) {
    return Column(
      children: [
        _buildModeSelector('Morning Bell Mode', provider.morningBellMode, provider.updateMorningBellMode),
        _buildModeSelector('Interval Bell Mode', provider.intervalBellMode, provider.updateIntervalBellMode),
        _buildModeSelector('Closing Bell Mode', provider.closingBellMode, provider.updateClosingBellMode),
      ],
    );
  }

  Widget _buildModeSelector(String title, List<int> mode, Function(int, int) onUpdate) {
    return Column(
      children: [
        const Divider(),
  ...
  Widget _buildModeDropdown(List<int> mode, int index, int itemCount, Function(int, int) onUpdate) {
    return DropdownButton<int>(
      value: mode[index],
      items: List.generate(itemCount, (i) => DropdownMenuItem(
        value: i,
